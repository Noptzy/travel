    ...
    Column(Modifier.fillMaxSize().background(SenjaCanvas)) {
        GradientHeader {
            Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
                Text("Buat akun baru", style = MaterialTheme.typography.headlineMedium, color = Color.White)
                Text("Simpan profil dan rencana perjalananmu.", color = Color.White.copy(alpha = 0.82f), style = MaterialTheme.typography.bodyMedium)
        ...
        Surface(
            ...
            Column(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                OutlinedTextField(
                    value = state.name,
                    onValueChange = { viewModel.onAction(AuthAction.NameChanged(it)) },
                    label = { Text("Nama") },
                ...
                PrimaryButton("Daftar", { viewModel.onAction(AuthAction.SubmitRegister) }, loading = state.isLoading)
